...
describe('App: chileAyudaMapaApp', function () {
  ...
  beforeEach(inject(function (API, $rootScope, $injector, _$location_, _$route_) {
    ...
    httpBackend.whenGET('views/reports_details.html').respond('fake_content');
  ...
  it('should report details route have incident and report id in params', function () {
    location.path('/incidents/1/reports/2');
    rootScope.$digest();

    expect(routeProvider.current.params.incidentId).toBe('1');
    expect(routeProvider.current.params.reportId).toBe('2');
  });
